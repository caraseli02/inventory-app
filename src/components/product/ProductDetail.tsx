...
const ProductDetail = ({ barcode, onScanNew }: ProductDetailProps) => {
  ...
  return (
    <Card className="w-full animate-in fade-in duration-500 shadow-none border-none border-stone-200 relative">
      ...
      <CardContent className="px-4 py-4 lg:px-6 lg:py-5 pb-24 md:pb-6 overflow-y-auto">
        ...
        <div className="lg:flex lg:gap-6">
          ...
          <div className="flex-1 lg:max-w-md">
            <div className="grid grid-cols-2 gap-3 mb-4">
              <Card className="bg-gradient-to-br from-stone-50 to-white border-2 border-stone-200">
                <CardContent className="p-3">
                  <div className="text-[10px] text-stone-500 uppercase tracking-widest font-bold mb-1">{t('product.price')}</div>
                  <div className="text-stone-900 font-semibold text-base">{displayPrice}</div>
                </CardContent>
              </Card>
              <Card className="bg-gradient-to-br from-stone-50 to-white border-2 border-stone-200">
                <CardContent className="p-3">
                  <div className="text-[10px] text-stone-500 uppercase tracking-widest font-bold mb-1">{t('product.expiry')}</div>
                  <div className="text-stone-900 font-semibold text-xs">{expiryDisplay}</div>
                </CardContent>
              </Card>
            </div>
            ...
            {/* Quantity Controls */}
            <div className="mb-3">
              <div className="text-[10px] text-stone-500 uppercase tracking-widest font-bold mb-2">{t('product.adjustQuantity')}</div>
              <div className="flex items-center justify-center gap-2">
                <Button
                  ...
                  variant="outline"
                  size="lg"
                  className="w-12 h-12 text-xl font-bold border-2 border-stone-300 hover:border-stone-400"
                >
                  −
                </Button>
                <div className="w-20">
                  <Input
                    ...
                    className="h-12 text-center text-xl font-bold border-2 border-stone-300 focus-visible:ring-[var(--color-lavender)]"
                  />
                </div>
                <Button
                  onClick={() => {
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
              <Button
                ...
                size="default"
                className="font-semibold bg-gradient-to-br from-[var(--color-forest)] to-[var(--color-forest-dark)] hover:opacity-90 text-white text-sm"
              >
                {loadingAction === 'IN' ? (
              ...
              <Button
                ...
                variant="outline"
                size="default"
                className="font-semibold border-2 border-[var(--color-terracotta)] text-[var(--color-terracotta)] hover:bg-[var(--color-terracotta)] hover:text-white disabled:opacity-50 disabled:cursor-not-allowed text-sm"
              >
                {loadingAction === 'OUT' ? (
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              <Button
                ...
                size="default"
                className="font-semibold border-2 border-stone-300 hover:bg-stone-100 flex items-center justify-center gap-1.5 text-sm"
              >
              ...
              <Button
                ...
                size="default"
                className="font-semibold border-2 border-[var(--color-terracotta)] text-[var(--color-terracotta)] hover:bg-red-50 flex items-center justify-center gap-1.5 text-sm"
              >
                🗑️ {t('product.delete')}
              </Button>
            </div>
            ...
            {currentStock < parseInt(stockQuantity) && (
              <div className="mb-3 p-2 bg-red-50 border-2 border-red-200 rounded-lg">
          ...
          {/* Right Column: Recent Activity (shows on lg+ OR below controls on mobile) */}
          <div className="lg:w-72 xl:w-80 lg:border-l-2 lg:border-stone-200 lg:pl-6 border-t-2 lg:border-t-0 border-stone-200 pt-4 lg:pt-0 mt-4 lg:mt-0">
            <h3 className="text-xs font-bold text-stone-900 uppercase tracking-wider mb-3">{t('product.recentActivity')}</h3>
            <div className="space-y-2 lg:max-h-[calc(100vh-400px)] lg:overflow-y-auto">
              {recentHistory.map((move) => (
                <div key={move.id} className="flex justify-between items-center text-sm p-2 bg-stone-50 rounded-lg border border-stone-200">
