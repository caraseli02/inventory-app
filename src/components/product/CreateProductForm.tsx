...
import { useCreateProduct, type AiStatus, type CreateProductFormData, type UseCreateProductResult } from '@/hooks/useCreateProduct';
...
function AiStatusBadge({ status }: { status: AiStatus }) {
  ...
  if (status === 'found') {
    return <Badge variant="secondary" className="bg-emerald-50 text-emerald-700 border-2 border-emerald-200 shadow-sm text-xs">{t('product.aiStatusFound', 'AI details found')}</Badge>;
  }
  if (status === 'not_found') {
    return <Badge variant="secondary" className="bg-zinc-100 text-zinc-700 border-2 border-zinc-200 shadow-sm text-xs">{t('product.aiStatusNotFound', 'No AI match, fill manually')}</Badge>;
  }
  if (status === 'error') {
    return <Badge variant="secondary" className="bg-amber-50 text-amber-700 border-2 border-amber-200 shadow-sm text-xs">{t('product.aiStatusError', 'AI unavailable, fill manually')}</Badge>;
  }
  return null;
}

function CreateProductLoadingOverlay() {
  const { t } = useTranslation();
  return (
...
function ImageSection({ imageUrl, name, setCameraOpen }: ImageSectionProps) {
  const { t } = useTranslation();
  return (
    <div className="flex flex-col items-center gap-3 pb-4 border-b border-stone-200">
      {imageUrl ? (
        <ProductImage src={imageUrl} alt={name || t('product.preview')} size="xl" showZoom />
      ) : (
        <Button type="button" variant="ghost" onClick={() => setCameraOpen(true)}
      ...
      {imageUrl && (
        <Button type="button" variant="outline" size="sm" onClick={() => setCameraOpen(true)} className="border-stone-300">
    ...
    <div className="space-y-4 pt-4 border-t border-stone-200">
      <div>
        ...
        <Select value={value} onValueChange={onValueChange}>
          ...
          <SelectContent>
            {(['General', 'Produce', 'Dairy', 'Meat', 'Pantry', 'Snacks', 'Beverages', 'Household', 'Conserve', 'Cereale'] as const).map((cat) => (
...
function PricingSection({ price, markup, basePrice, storePrice, onPriceChange, onMarkupChange }: PricingSectionProps) {
  const { t } = useTranslation();
  return (
    <div className="space-y-4 pt-4 border-t border-stone-200">
      ...
      <div>
        ...
        <div className="relative mt-2">
          ...
          <Input id="price" type="number" name="price" step="0.01" min="0" value={price} onChange={onPriceChange} placeholder="0.00"
      ...
      <div>
        ...
        <div className="flex rounded-lg border-2 border-stone-200 bg-stone-50 p-1">
          {([50, 70, 100] as MarkupPercentage[]).map((option) => (
            <Button key={option} type="button" variant="ghost" size="sm" onClick={() => onMarkupChange(option)}
              className={`flex-1 h-10 font-semibold transition-all ${markup === option ? 'bg-[var(--color-forest)] text-white hover:bg-[var(--color-forest-dark)] hover:text-white' : 'text-stone-600 hover:bg-stone-200 hover:text-stone-900'}`}>
              {option}%
            </Button>
          ))}
        </div>
      ...
      {basePrice != null && storePrice != null && (
        <div className="p-3 bg-[var(--color-forest)]/10 border-2 border-[var(--color-forest)]/30 rounded-lg">
          ...
          <p className="text-xs text-[var(--color-forest)] mt-1">
            {t('markup.formula', { markup, base: basePrice.toFixed(2), store: storePrice.toFixed(2) })}
          </p>
        </div>
      )}
    </div>
...
interface FormBodyProps {
  ...
  setFormData: UseCreateProductResult['setFormData'];
  nameError: boolean;
  setNameError: UseCreateProductResult['setNameError'];
  nameInputRef: UseCreateProductResult['nameInputRef'];
  barcode: string;
  basePrice: number | null;
  storePrice: number | null;
  handleSubmit: (e: FormEvent<HTMLFormElement>) => void;
  handleChange: UseCreateProductResult['handleChange'];
  setCameraOpen: (open: boolean) => void;
}

function CreateProductFormBody({ formId, formData, setFormData, nameError, setNameError, nameInputRef, barcode, basePrice, storePrice, handleSubmit, handleChange, setCameraOpen }: FormBodyProps) {
  const { t } = useTranslation();
  return (
    <form onSubmit={handleSubmit} className="space-y-6" id={formId}>
      <ImageSection imageUrl={formData.imageUrl} name={formData.name} setCameraOpen={setCameraOpen} />
      <div className="space-y-4">
        <div>
          <Label htmlFor="barcode" className="text-stone-700 font-semibold text-sm">{t('product.barcode')}</Label>
          <Input id="barcode" type="text" value={barcode} disabled className="mt-2 h-11 bg-stone-50 border-2 border-stone-300 text-stone-600 cursor-not-allowed" />
        ...
        <div>
          ...
          <Input ref={nameInputRef} id="name" type="text" name="name" required value={formData.name}
            onChange={(e) => {
            ...
            className={`mt-2 h-11 border-2 focus-visible:ring-[var(--color-lavender)] focus-visible:border-[var(--color-lavender)] ${nameError ? 'border-red-500 focus-visible:ring-red-500 focus-visible:border-red-500' : 'border-stone-300'}`} />
          {nameError
      ...
      <CategorySelect value={formData.category} onValueChange={(value) => setFormData(prev => ({ ...prev, category: value }))} />
      <PricingSection price={formData.price} markup={formData.markup} basePrice={basePrice} storePrice={storePrice}
        onPriceChange={handleChange} onMarkupChange={(m) => setFormData(prev => ({ ...prev, markup: m }))} />
      <div className="space-y-4 pt-4 border-t border-stone-200">
        <div>
          <Label htmlFor="initialStock" className="text-stone-700 font-semibold text-sm">{t('product.initialStock')}</Label>
          <Input id="initialStock" type="number" name="initialStock" min="0" value={formData.initialStock} onChange={handleChange}
      ...
      <div className="space-y-4 pt-4 border-t border-stone-200">
        <div>
          ...
          <div className="flex gap-2 mt-2">
            <Input id="imageUrl" type="url" name="imageUrl" value={formData.imageUrl} onChange={handleChange}
...
function CreateProductForm({ barcode, onSuccess, onCancel }: CreateProductFormProps) {
  ...
  const formId = useId();
  const {
    formData, setFormData, nameError, setNameError, cameraOpen, setCameraOpen,
    displayAiStatus, basePrice, storePrice, isFormValid,
    nameInputRef, isPending, isError, error, handleSubmit, handleChange,
  } = useCreateProduct(barcode, onSuccess);
  ...
  return (
    <Card className="w-full max-w-lg mx-auto animate-in slide-in-from-bottom-5 duration-300 shadow-lg border-2 border-stone-200 relative">
      {isPending && <CreateProductLoadingOverlay />}
      <CardHeader className={`bg-gradient-to-br from-stone-50 to-stone-100/50 border-b-2 border-stone-200 px-4 py-2 sm:px-6 sm:py-4 ${isPending ? 'invisible' : ''}`}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg sm:text-2xl font-bold text-stone-900">{t('product.createProduct')}</CardTitle>
          <AiStatusBadge status={displayAiStatus} />
        </div>
      </CardHeader>
      <CardContent className={`px-6 py-6 max-h-[calc(100dvh-280px)] md:max-h-none overflow-y-auto ${isPending ? 'invisible' : ''}`}>
        <CreateProductFormBody
          formId={formId} formData={formData} setFormData={setFormData}
          nameError={nameError} setNameError={setNameError} nameInputRef={nameInputRef}
          barcode={barcode} basePrice={basePrice} storePrice={storePrice}
          handleSubmit={handleSubmit} handleChange={handleChange} setCameraOpen={setCameraOpen}
        />
        {isError && (
          <div className="mt-6 text-red-700 text-sm bg-red-50 p-4 rounded-lg border-2 border-red-200 font-medium flex items-start gap-2">
            ...
            <span>{error?.message || t('errors.productCreationFailed')}</span>
          </div>
        )}
      </CardContent>
      <CardFooter className={`bg-gradient-to-br from-stone-50 to-stone-100/50 p-4 sm:p-6 border-t-2 border-stone-200 flex gap-3 fixed md:static bottom-0 w-full ${isPending ? 'invisible' : ''}`}>
        <Button type="button" onClick={onCancel} variant="outline" className="hidden sm:flex flex-1 border-2 border-stone-300 hover:bg-stone-100 font-semibold h-12">
          {t('product.cancel')}
        </Button>
        <Button type="submit" form={formId} formNoValidate disabled={isPending || !isFormValid}
      ...
      <CameraCaptureDialog open={cameraOpen} onOpenChange={setCameraOpen}
        onCapture={(imageDataUrl) => setFormData(prev => ({ ...prev, imageUrl: imageDataUrl }))} />
    </Card>
  );
}
